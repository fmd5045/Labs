...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
	    ...
	    System.out.println("-----> All of the cards in the deck <-----");
	    createdDeck.describe();
		...
		// Test your method here
		Card drawnCard =createdDeck.draw();
		drawnCard.describe();
		...
		for(int i=0; i<numOfPlayers; i++) {
			...
			//do we actually need another for loop in order to get the players hand
			List<Card> playersHand = gameBoard.get(playerName);
	...
	public static Map<String,List<Card>> createGame(int numOfPlayers) {
		...
		newDeck.shuffle();
		int cardPerPlayer = newDeck.lenght()/numOfPlayers;
		System.out.println(">>>>>> GAME SETUP <<<<<<");
		System.out.println("Total Cards on Deck: "+newDeck.lenght());
		...
		for (int i= 0; i<numOfPlayers; i++) {
			...
			for(int j=0; j<cardPerPlayer; j++) {
				playerCards.add(newDeck.draw());
			}
			finalGameBoard.put(player, playerCards);
		}
